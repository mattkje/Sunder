"""Loads a Mel-Band-Roformer checkpoint into a MelBandRoformer built from its
own config. Parametrized by a checkpoint directory (containing config.yaml
and model.ckpt) -- generalizes load_model.py (which stays as-is, hardcoded
to becruily_deux, since build_coreml.py/verify.py already depend on it)
for every other Mel-Band-Roformer checkpoint.
"""
