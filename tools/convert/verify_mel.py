"""Parity checks for the Mel-Band-Roformer wrapper against an arbitrary
checkpoint (generalizes verify.py, which stays hardcoded to becruily_deux).
Usage:

  .venv/bin/python verify_mel.py wrapper <checkpoint_dir>
  .venv/bin/python verify_mel.py coreml <checkpoint_dir> <mlpackage_path> [chunk_size]
...
def make_test_audio(chunk_size):
    sr = 44100
    n = chunk_size
    torch.manual_seed(0)
